...
select 
    ...
    year(B.create_time) as year, week(B.create_time) as week
...
join (
    ...
    join (
        ...
        from exchange_balance
        group by address, currency, year(create_time), week(create_time)
    ) A  
...
group by exchange, currency, year(B.create_time), week(B.create_time)
"""
...
select 
    ...
    year(create_time) as year, week(create_time) as week
...
join (
    select
        ...
        max(create_time) as max_create_time
    from exchange_btc_balances
    group by exchange, year(create_time), week(create_time)
) A  
...
for row in erc20_balances:
    ...
    key = exchange + str(year) + str(week)
    ...
    if exchange_balance_dict:
        ...
        reserves_balance_dict[key] = {'exchange': exchange, currency.upper() + '_reserves': balance, 'update_time': update_time}
...
for row in btc_balances:
    ...
    if exchange_dict:
        ...
        reserves_balance_dict[key] = {'exchange': exchange, 'BTC_reserves': balance, 'update_time': update_time}
...
reserves_list.sort(key=lambda d: d['market'] + str(d['update_time']))

with open('reserves_weekly.csv', 'w') as file:
    writer = csv.DictWriter(file, ['market', 'BTC_reserves', 'ETH_reserves', 'USDT_reserves', 'update_time'])
    writer.writeheader()
